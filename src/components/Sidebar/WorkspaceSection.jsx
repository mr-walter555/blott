...
export default function WorkspaceSection() {
  ...
  return (
    <div className="space-y-0.5 px-0 py-1">
          ...
          <DropdownMenu anchor={menuTriggerRef} open={menuId === ws.id} onClose={() => setMenuId(null)} align="right">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl py-1 w-36">
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-1.5 text-sm hover:bg-red-50 dark:hover:bg-red-950/40 text-red-500 dark:text-red-400"
              >
      ...
      <button
        onClick={openCreate}
        className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-muted dark:text-gray-600 hover:bg-gray-100 dark:hover:bg-white/[0.06] hover:text-gray-600 dark:hover:text-muted transition-colors"
      >
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
        <motion.div
          ...
          className="pointer-events-auto w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl ring-1 ring-black/5 dark:ring-white/10 overflow-hidden"
        >
          {/* Header */}
                ...
                {PALETTE.map(c => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full transition-transform hover:scale-110 focus:outline-none ${
                      color === c ? 'ring-2 ring-offset-2 ring-offset-white dark:ring-offset-gray-900' : ''
                    }`}
                    style={{ background: c, ...(color === c ? { '--tw-ring-color': c } : {}) }}
                    title={c}
                    aria-label={`Color ${c}`}
                  />
                ))}
