...
import { Sparkle, Article, PencilSimple, TextT, ListBullets, ListChecks, ArrowsOutSimple, ArrowsInSimple } from '@phosphor-icons/react'
...
>
      ...
      <div className="flex items-center gap-2 px-3.5 py-2.5 border-b border-gray-100 dark:border-gray-800">
        <div className="w-5 h-5 rounded-md bg-brown-100 dark:bg-brown-950/40 flex items-center justify-center flex-shrink-0">
          <Sparkle className="w-3 h-3 text-brown-600 dark:text-brown-400" />
        </div>
        <span className="text-xs font-semibold text-gray-700 dark:text-muted tracking-wide">AI Actions</span>
      </div>
      ...
      <div className="py-1">
        {AI_ACTIONS.map(({ id, label, icon: Icon, desc }) => (
          <button
            ...
            className="w-full flex items-center gap-3 px-3.5 py-2 text-left text-gray-700 dark:text-muted hover:bg-gray-50 dark:hover:bg-white/[0.06] transition-colors"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-muted dark:text-gray-600 leading-tight mt-0.5">{desc}</p>
            </div>
          </button>
        ))}
      </div>
