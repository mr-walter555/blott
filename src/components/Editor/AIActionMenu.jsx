...
export default function AIActionMenu({ onSelect }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-lg overflow-hidden w-56"
      style={{ fontFamily: 'Geist Variable, sans-serif' }}>

      {/* Header */}
      <div className="flex items-center gap-2 px-3.5 py-2.5 border-b border-gray-100">
        <div className="w-5 h-5 rounded-md bg-brown-100 flex items-center justify-center flex-shrink-0">
          <Sparkle className="w-3 h-3 text-brown-600" weight="fill" />
        </div>
        <span className="text-xs font-semibold text-gray-700 tracking-wide">AI Actions</span>
      </div>
      ...
      <div className="py-1">
        {AI_ACTIONS.map(({ id, label, icon: Icon, desc }) => (
          <button
            ...
            className="w-full flex items-center gap-3 px-3.5 py-2 text-left text-gray-700 hover:bg-gray-50 transition-colors"
          >
